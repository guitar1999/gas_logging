...
query = """INSERT INTO gas_usage_yearly (year, btu, complete, timestamp) VALUES (%s, 0, 'no', '%s-01-01 00:00:00');""" % (year, year)
cursor.execute(query)
db.commit()

query = """SELECT min(measurement_time)::date = '%s-01-01'::date, max(measurement_time)::date = '%s-12-31'::date, max(tdiff) < 300  FROM electricity_measurements WHERE date_part('year', measurement_time) = %s;""" % (opyear, opyear, opyear)
cursor.execute(query)
