...
BEGIN
   ...
   RETURN QUERY  WITH join_query AS (
        ...
        SELECT
            ...
            s2.status2,
            CASE 
                WHEN NOT s2.status2 = 'boiler and circulator' THEN NULL
                WHEN s2.status2 = 'boiler and circulator' AND NOT LAG(s2.status2) OVER (ORDER BY s2.measurement_time) = 'boiler and circulator' THEN date_part('epoch', s2.measurement_time)
                WHEN s2.status2 = 'boiler and circulator' AND LAG(s2.status2) OVER (ORDER BY s2.measurement_time) = 'boiler and circulator' THEN NULL
            END AS event_group 
        FROM 
            status2_query AS s2
        ...
        SELECT
            ...
            CASE
                WHEN e2.status2 = 'boiler and circulator' THEN ROUND((140000 * e2.tdiff / 60 / 60), 0)::numeric
                ELSE 0::numeric
            END AS btu, 
